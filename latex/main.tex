\documentclass[conference]{IEEEtran}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{algorithmic}
\usepackage{graphicx}
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{tikz}
\usetikzlibrary{arrows,automata,positioning,shapes.geometric,fit,calc}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{array}
\usepackage{url}
\usepackage{hyperref}
\graphicspath{{../output/figures/}}
\usepackage{cite}
\usepackage{balance}

\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}}
\newcolumntype{C}[1]{>{\centering\arraybackslash}p{#1}}

\hypersetup{
  colorlinks=true,
  linkcolor=black,
  citecolor=black,
  urlcolor=blue,
  bookmarks=true
}

\begin{document}
\title{%
  {\fontsize{15}{18}\selectfont
    The Machine Proposes. The Proof Disposes.}\\[4pt]
  {\fontsize{12}{15}\selectfont
    Neuro-Symbolic Synthesis of Formally Verified Markov Usage Models
    from Natural Language Requirements}}

\author{
  % All names in one block so IEEEtran centres them as a unit
  \IEEEauthorblockN{%
    Nathan G.\IEEEauthorrefmark{1}\IEEEauthorrefmark{2},\;
    Jaeden Ting YiYong\IEEEauthorrefmark{1},\;
    Wai Phyo Hein\IEEEauthorrefmark{1},\;
    Jordan Chay\IEEEauthorrefmark{1}}
  \IEEEauthorblockA{%
    \IEEEauthorrefmark{1}\textit{School of Computing and Artificial
    Intelligence, Sunway University}, Subang Jaya, Malaysia\\
    \{nathan.n,\,23009798,\,22108765,\,23009376\}@imail.sunway.edu.my}
  \IEEEauthorblockA{%
    \IEEEauthorrefmark{2}\textit{Mercedes-Benz Tech Innovation}\quad
    {\small\url{https://orcid.org/0009-0002-8492-8094}}}
}

\maketitle

\input{sections/abstract}
\input{sections/introduction}
\input{sections/literature_review}
\input{sections/related_work}
\input{sections/bottlenecks}
\input{sections/framework}
\input{sections/mathematical_formulations}
\input{sections/evaluation}
\input{sections/ablation}
\input{sections/threats}
\input{sections/conclusion}

\balance
\bibliographystyle{IEEEtran}
\bibliography{references}

\end{document}