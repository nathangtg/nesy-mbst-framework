...
chains~\cite{prowell_markov}, custom DSLs~\cite{whittaker1993markov,whittaker1994markov},
AAL annotations~\cite{boehr_constraints}, or full UML state
machines~\cite{pretschner2005mbt}. Constructing and maintaining these
...
\centering
\begin{tikzpicture}[
    >=latex, node distance=0pt,
    every node/.style={font=\small},
    inbox/.style={
        draw, rounded corners=3pt,
        minimum height=1.0cm, minimum width=2.4cm,
        align=center, fill=blue!8, draw=blue!50!black
    },
    container/.style={
        draw=gray!60, rounded corners=4pt,
        fill=gray!6, inner sep=10pt
    },
    resultbox/.style={
        draw, rounded corners=3pt,
...
%% Input node
\node[inbox] (prompt) {Natural Language\\Requirements};

%% LLM container — placed to the right with explicit gap
\node[container, right=1.6cm of prompt] (llmbox) {%
    \begin{minipage}{5.0cm}
        \centering
        {\small\bfseries Large Language Model}\\[6pt]
        \tikz{
            \node[resultbox, fill=green!8, draw=green!50!black] (s1)
                {Single-Prompt Baseline\\[2pt]
                 $F_1^{\text{St}}\!\approx\!0.80$,\;
                 $F_1^{\text{Tr}}\!\approx\!0.75$};
            \node[resultbox, fill=orange!8, draw=orange!50!black,
                  below=8pt of s1] (s2)
                {Multi-Step / Hybrid\\[2pt]
                 $F_1^{\text{St}}\!\approx\!0.85$,\;
                 $F_1^{\text{Tr}}\!\approx\!0.65$};
        }
    \end{minipage}%
};

%% Arrow with label above it
\draw[arr] (prompt.east) -- node[above, font=\scriptsize, yshift=2pt]
    {prompt} (llmbox.west);

\end{tikzpicture}
\caption{LLM pipeline for state-machine extraction. Single-prompt
baselines achieve higher transition $F_1$ but lower state $F_1$ than
multi-step hybrid strategies, motivating the NeSy-MBST active-learning
loop.}
