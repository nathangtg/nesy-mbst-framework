...
\begin{tikzpicture}[
    ...
    %  - node styles  -
    engine/.style={
        rectangle, draw=blue!60!black, rounded corners=3pt,
        fill=blue!7, minimum width=3.2cm, minimum height=1.2cm,
        text width=3.0cm, align=center, line width=0.7pt},
    oracle/.style={
        rectangle, draw=green!60!black, rounded corners=3pt,
        fill=green!7, minimum width=3.2cm, minimum height=1.2cm,
        text width=3.0cm, align=center, line width=0.7pt},
    sut/.style={
        rectangle, draw=red!60!black, rounded corners=3pt,
        fill=red!7, minimum width=3.2cm, minimum height=1.2cm,
        text width=3.0cm, align=center, line width=0.7pt},
    topo/.style={
        rectangle, draw=yellow!70!black, rounded corners=3pt,
        fill=yellow!12, minimum width=3.2cm, minimum height=1.0cm,
        text width=3.0cm, align=center, line width=0.7pt},
    solver/.style={
        rectangle, draw=orange!70!black, rounded corners=3pt,
        fill=orange!8, minimum width=3.2cm, minimum height=1.2cm,
        text width=3.0cm, align=center, line width=0.7pt},
    outnode/.style={
        rectangle, draw=violet!60!black, rounded corners=3pt,
        fill=violet!7, minimum width=3.2cm, minimum height=1.2cm,
        text width=3.0cm, align=center, line width=0.7pt},
    %  - arrow styles  -
    arr/.style={->, thick},
...
%% ── Column coordinates ──────────────────────────────────────────────────────
%% Left col  x=0   (ALE, Topo, Solver, Output  — vertical spine)
%% Mid  col  x=4.6 (anchor for horizontal label helpers)
%% Right col x=5.0 (Oracle row 1, SUT row 2)

%% Row 1
\node[engine]  (ale)    at (0,   0)    {Active Learning Engine\\{\scriptsize ($L^{*}$ Algorithm)}};
\node[oracle]  (oracle) at (5.0, 0)    {Grammar-Constrained\\LLM Oracle\\{\scriptsize Out: \{Yes,\,No,\,Unsure\}}};

%% Row 2
\node[sut]     (sut)    at (5.0,-2.6)  {SUT Execution\\Validation\\{\scriptsize Returns counterexamples}};
\node[topo]    (topo)   at (0,  -2.6)  {Verified State\\Topology};

%% Row 3
\node[solver]  (solver) at (0,  -5.0)  {Symbolic Constraint\\Solver\\{\scriptsize Convex equations}};

%% Row 4
\node[outnode] (out)    at (0,  -7.2)  {Calibrated Markov Usage\\Model $\to$ Executable\\Test Sequences};

%% ── Arrows ──────────────────────────────────────────────────────────────────

% ALE → Oracle  (horizontal right)
\draw[arr] (ale.east) -- node[lbl, above] {membership queries} (oracle.west);

% Oracle → Topo  (drop straight down on the right spine)
\draw[arr] (oracle.south) -- node[lbl, right] {responses} (sut.north);

% ALE → SUT  (horizontal right then elbow down — fully orthogonal, no overlap)
\draw[arr] (ale.south) -- ++(0,-0.6) -| node[lbl, right, pos=0.15] {equiv.\ queries} (sut.west);

% SUT → Topo  (horizontal left)
\draw[arr] (sut.west) -- node[lbl, above] {counterexamples} (topo.east);
...
% Feedback  (elbow left of spine, fully orthogonal)
\draw[darr] (out.west) -- ++(-1.2,0) |- node[lbl, left, pos=0.25] {runtime feedback} (ale.west);

\end{tikzpicture}
\caption{Architecture of the NeSy-MBST framework. The Active Learning Engine
(left spine) issues membership queries to the grammar-constrained LLM oracle
(right, row 1) and equivalence queries to SUT execution (right, row 2).
Both return orthogonally to the verified state topology, which feeds the
symbolic constraint solver and the calibrated Markov usage model. A dashed
elbow path carries runtime telemetry back to the learning engine.}%
...
To systematically discover the state space of the SUT, NeSy-MBST deploys a specialized variant of the $L^{*}$ active automata learning algorithm~\cite{angluin1987,lstar_llm}. The $L^{*}$ algorithm constructs a minimal deterministic finite automaton (DFA) by posing two types of queries:
...
Through iterative refinement, the $L^{*}$-based loop constructs a verified state topology $\mathcal{A} = (Q, \Sigma, \delta, q_0)$ representing the reachable states and transitions of the SUT~\cite{lstar_llm}.  The membership oracle answers \emph{reachability} queries (``is state sequence $\sigma$ executable?''), not language-acceptance queries, so the learned structure is the \emph{support topology} of the target Markov chain rather than a classical DFA accepting language.  In our proof-of-concept evaluation across all ablation conditions, the active learning loop converged in every run; on the AV benchmark, 94\% of membership queries were resolved directly by the oracle, with 6\% escalated to SUT execution for empirical resolution.  No \texttt{Unsure} responses were observed on the AV benchmark, consistent with the specificity of its natural-language requirements.  Convergence under a three-valued oracle is empirically verified but not formally proven; the Angluin~\cite{angluin1987} convergence guarantee applies when the oracle provides consistent binary responses.

%%%  - Subsection C  - %%%
\subsection{Path-Dependent Hierarchical Modeling}%
\label{subsec:hierarchical}

A first-order Markov chain is the standard representation in classical MBST~\cite{whittaker1993markov,whittaker1994markov} where it assumes that the probability of transitioning to the next state depends solely on the current state. In practice, many software behaviors exhibit \emph{path-dependent} patterns: the likelihood of a user action depends not only on the current screen but also on the sequence of prior interactions~\cite{usage_model_web}.
...
By decoupling constraint extraction (a natural-language task suited to LLMs) from constraint resolution (a mathematical task suited to solvers), NeSy-MBST ensures that the resulting probability matrix is both semantically informed and numerically exact~\cite{boehr_constraints, whittaker1993markov}.
